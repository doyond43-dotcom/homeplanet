...
fs.writeFileSync(path + ".before-payment-proof-review-restore", raw);

// Add proof text helper
...
// Make work/photos next move smarter after photos are saved
raw = raw.replace(
  "Next Move: Take before and after photos if needed so the job proof stays with the request.",
  `{afterPhotos.length ? "Next Move: Photos are saved. Send payment link, then request review/share proof." : "Next Move: Take before and after photos if needed so the job proof stays with the request."}`
);

// Insert Payment + Proof/Review before Timeline if missing
...
if (!raw.includes("PAYMENT") || !raw.includes("PROOF / REVIEW")) {
