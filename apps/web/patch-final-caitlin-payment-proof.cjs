...
fs.writeFileSync(path + ".before-final-payment-proof-insert", raw);

if (!raw.includes("function buildProofText")) {
  const insertAt = raw.indexOf("function buildSocialPost");
  const helper = `function buildProofText(signal: Signal, beforePhotos: CleaningPhoto[], afterPhotos: CleaningPhoto[]) {
...
`;
  raw = raw.slice(0, insertAt) + helper + raw.slice(insertAt);
}

const timelineLabel = raw.indexOf("TIMELINE");
const timelineCard = raw.lastIndexOf('              <div className="rounded-2xl border border-white/10 bg-zinc-950 p-4">', timelineLabel);

if (timelineLabel === -1 || timelineCard === -1) {
  throw new Error("Could not find timeline card.");
}
...
raw = raw.slice(0, timelineCard) + finishSections + raw.slice(timelineCard);
