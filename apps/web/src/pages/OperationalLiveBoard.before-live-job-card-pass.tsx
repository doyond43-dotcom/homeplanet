...
export default function OperationalLiveBoard({ boardSlug, payload }: Props) {
  ...
  const [draftJob, setDraftJob] = useState<OperationalJob | null>(null);
            ...
            {selectedJob ? (
              <>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" }}>
                  ...
                  {isEditing ? (
                    <div style={{ display: "flex", gap: 8 }}>
                      <button style={primaryButton} onClick={saveJobChanges}>Save</button>
                      <button style={dangerButton} onClick={cancelEditing}>Cancel</button>
                    </div>
                  ) : (
                    <button style={button} onClick={startEditing}>Edit Details</button>
                  )}
                </div>
                ...
                {isEditing ? (
                  <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
                    <div>
                      ...
                      <input style={{ ...field, marginTop: 6, fontSize: 20 }} value={draftJob?.customer || ""} onChange={(event) => updateDraft("customer", event.target.value)} />
                    </div>

                    <div>
                      <div style={label}>Service / Job Title</div>
                      <input style={{ ...field, marginTop: 6 }} value={draftJob?.service || ""} onChange={(event) => updateDraft("service", event.target.value)} />
                    </div>
                  </div>
                ) : (
                  <>
                ...
                <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <button style={button} onClick={() => setShowProofSection((open) => !open)}>
                    {showProofSection ? "Hide Proof" : "Upload Proof"}
                  </button>

                  <button style={button} onClick={() => setShowPaymentSection((open) => !open)}>
                    {showPaymentSection ? "Hide Payment" : "Collect Payment"}
                  </button>

                  <button style={button} onClick={() => setShowMoveSection((open) => !open)}>
                    {showMoveSection ? "Hide Stages" : "Move Stage"}
                  </button>

                  <button style={button} onClick={() => setShowTimelineSection((open) => !open)}>
                    {showTimelineSection ? "Hide Timeline" : "View Timeline"}
                  </button>
                </div>

                <div style={{ display: "grid", gap: 10, marginTop: 14 }}>
                  <div style={{ borderRadius: 16, background: "rgba(0,0,0,0.25)", padding: 12 }}>
                    <div style={label}>Customer Info</div>

                    {isEditing ? (
                      <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                        <input style={field} placeholder="Phone" value={draftJob?.phone || ""} onChange={(event) => updateDraft("phone", event.target.value)} />
                        <input style={field} placeholder="Email" value={draftJob?.email || ""} onChange={(event) => updateDraft("email", event.target.value)} />
                        <input style={field} placeholder="Address" value={draftJob?.address || ""} onChange={(event) => updateDraft("address", event.target.value)} />
                      </div>
                    ) : (
                      <div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.7 }}>
                        Phone: <strong>{selectedJob.phone || "Not added"}</strong>
                        <br />
                        Email: <strong>{selectedJob.email || "Not added"}</strong>
                        <br />
                        Address: <strong>{selectedJob.address || "Not added"}</strong>
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ borderRadius: 16, background: "rgba(0,0,0,0.25)", padding: 12 }}>
                    ...
                    {isEditing ? (
                      <textarea
                        style={{ ...field, marginTop: 8, minHeight: 110, resize: "vertical", lineHeight: 1.5 }}
                  ...
                  {showProofSection ? (
                    <div style={{ borderRadius: 16, background: "rgba(0,0,0,0.25)", padding: 12 }}>
                      <div style={label}>Photo Proof</div>
                  ...
                  {showTimelineSection ? (
                    <div style={{ borderRadius: 16, background: "rgba(0,0,0,0.25)", padding: 12 }}>
                      <div style={label}>Proof Timeline</div>
