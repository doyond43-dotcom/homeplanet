...
const STORAGE_KEY = "homeplanet-florida-cooling-board-v1";
...
export default function FloridaCoolingBoardPage() {
  const [requests, setRequests] = useState<CoolingRequest[]>(() => {
    if (typeof window === "undefined") return startingRequests;

    try {
      const saved = window.localStorage.getItem(STORAGE_KEY);
      if (!saved) return startingRequests;

      const parsed = JSON.parse(saved) as CoolingRequest[];
      return Array.isArray(parsed) && parsed.length > 0 ? parsed : startingRequests;
    } catch {
  ...
  function resetDemoBoard() {
    setRequests(startingRequests);
    setActiveId(startingRequests[0].id);
    setReplyAction("Follow Up");
    setCopied(false);
    setSaved(false);
    setLastAction("Demo board reset.");
  }
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                className="inline-flex w-fit items-center justify-center rounded-2xl border border-red-400/25 bg-red-500/10 px-5 py-3 text-sm font-black text-red-200 transition hover:bg-red-500/20"
              >
                Reset Demo Board
              </button>
            </div>
