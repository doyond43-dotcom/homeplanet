...
export default function LivePagesHub() {
  ...
  const nav: CSSProperties = {
    ...
    background:
      "radial-gradient(circle at 12% 50%, rgba(59,130,246,0.14), transparent 28%), radial-gradient(circle at 80% 50%, rgba(52,211,153,0.08), transparent 26%), rgba(0,0,0,0.58)",
    padding: "11px 14px 11px 16px",
    boxShadow: "0 18px 60px rgba(0,0,0,0.38), inset 0 1px 0 rgba(255,255,255,0.05)",
  };
  ...
  const mark: CSSProperties = {
    ...
    placeItems: "center",
    background:
      "radial-gradient(circle at 35% 30%, rgba(96,165,250,0.26), transparent 32%), rgba(2,6,23,0.92)",
    border: "1px solid rgba(96,165,250,0.22)",
    boxShadow:
      "0 0 24px rgba(96,165,250,0.30), 0 0 18px rgba(52,211,153,0.16), inset 0 1px 0 rgba(255,255,255,0.08)",
    overflow: "hidden",
  };

  const planetRing: CSSProperties = {
    position: "absolute",
    width: 38,
    height: 14,
    borderRadius: 999,
    border: "1.5px solid rgba(251,191,36,0.88)",
  ...
  const planetCore: CSSProperties = {
    ...
    borderRadius: 999,
    background:
      "radial-gradient(circle at 34% 28%, #dbeafe, #60a5fa 24%, #1d4ed8 48%, #020617 82%)",
    border: "1px solid rgba(147,197,253,0.35)",
    boxShadow:
      "0 0 18px rgba(59,130,246,0.42), inset 0 0 10px rgba(255,255,255,0.16)",
  };
  ...
  const navBtn: CSSProperties = {
    ...
    background:
      "linear-gradient(135deg, rgba(52,211,153,0.95), rgba(16,185,129,0.86))",
    color: "#04110c",
    padding: "10px 15px",
    textDecoration: "none",
    fontWeight: 1000,
  ...
  return (
    <main style={page}>
      <div style={wrap}>
        <nav style={nav}>
          <Link to="/planet/live-pages" style={brand}>
            <span style={mark} aria-hidden="true">
              <span style={planetRing} />
              <span style={planetCore} />
            </span>
            <span>
              <span style={{ display: "block", fontWeight: 1000, fontSize: 15, lineHeight: 1 }}>
                HomePlanet
              </span>
              <span style={{ display: "block", marginTop: 3, color: "#93c5fd", fontSize: 11, fontWeight: 850, letterSpacing: "0.08em" }}>
                LIVE PAGES FOR LOCAL BUSINESS
              </span>
            </span>
          </Link>
        ...
        <section style={section}>
          <div style={eyebrow}>LIVE BUILD QUEUE</div>
          <h2 style={h2}>Built in a real queue, not a black hole.</h2>
          <p style={{ ...sub, fontSize: 15 }}>
            When you message us, your request moves through a simple build flow. You are not guessing
            where things stand. We build in order, keep the workflow clear, and move each Live Page
            System toward review.
          </p>
            ...
            {["Message received", "Info confirmed", "First version building", "Ready for review", "Approved and live"].map(
              (step, index) => (
                <article key={step} style={card}>
                  ...
                  <strong style={{ display: "block", marginTop: 10 }}>{step}</strong>
                </article>
              )
            )}
          </div>
