...
export default function JonesEquipmentOperatorBoard() {
  ...
  const visibleRentalRequests =
    showAllRentals
      ? rentalRequests
      : rentalRequests.slice(0, 3);

  const visibleRepairRequests =
    showAllRepairs
      ? repairRequests
      : repairRequests.slice(0, 3);
    ...
    <main className="jme-board-page">
      <style>{`
        ...
        * {
        ...
        .jme-board-card-list::-webkit-scrollbar {
          width: 8px;
        }

        .jme-board-card-list::-webkit-scrollbar-track {
          background: transparent;
        }

        .jme-board-card-list::-webkit-scrollbar-thumb {
        ...
        .jme-board-show-more span:last-child {
          color: #7e8b94;
          font-size: 0.75rem;
        }
        .jme-board-empty,
          ...
          <div className="jme-board-lanes">
            <section className="jme-board-lane rental">
              ...
              {rentalRequests.length > 3 ? (
                <button
                  className="jme-board-show-more"
                  type="button"
                  onClick={() =>
                    setShowAllRentals((current) => !current)
                  }
                >
                  <span>
                    {showAllRentals
                      ? "Show fewer rental requests"
                      : `Show ${
                          rentalRequests.length - 3
                        } more rental request${
                          rentalRequests.length - 3 === 1
                            ? ""
                            : "s"
                        }`}
                  </span>
                  ...
                  <span>
                    {showAllRentals ? "Collapse" : "Expand"}
                  </span>
                </button>
              ) : null}
            </section>
            ...
            <section className="jme-board-lane repair">
              ...
              {loading ? (
                ...
                <div className="jme-board-card-list">
                  {visibleRepairRequests.map((request) => (
                    <button
                      key={request.id}
                      className="jme-board-card"
                      type="button"
              ...
              {repairRequests.length > 3 ? (
                ...
                >
                  <span>
                    {showAllRepairs
                      ? "Show fewer repair requests"
                      : `Show ${
                          repairRequests.length - 3
                        } more repair request${
                          repairRequests.length - 3 === 1
                            ? ""
                            : "s"
                        }`}
                  </span>
                  ...
                  <span>
                    {showAllRepairs ? "Collapse" : "Expand"}
                  </span>
                </button>
              ) : null}
            </section>
