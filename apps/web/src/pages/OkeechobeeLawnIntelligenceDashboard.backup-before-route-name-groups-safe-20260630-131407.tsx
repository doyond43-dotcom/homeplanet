...
What days or times usually work best, and is there anything else we should know before we come by?`;
  ...
  const routeStatuses = ["New", "Contacted", "Scheduled", "Done"];
      ...
      <div className="mt-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3">
        ...
        <div className="mt-2 grid gap-2 md:grid-cols-4">
          ...
          <button
            ...
            onClick={() =>
              isScheduledOnRoute
                ? handleRouteStatus("Contacted")
                : handleRouteStatus("Scheduled")
            }
            disabled={
              routeSavingStatus === "Scheduled" ||
              routeSavingStatus === "Contacted"
            }
            className={[
              "rounded-xl px-4 py-2 text-xs font-black transition",
              routeSavingStatus === "Scheduled" || routeSavingStatus === "Contacted"
                ? "cursor-wait bg-green-400/40 text-black/60"
                : isScheduledOnRoute
                  ? "border border-yellow-300/30 bg-yellow-300/10 text-yellow-200 hover:bg-yellow-300/15"
