...
function resolveLiveBoardConfig(args: {
  ...
  if (bt.includes("home services")) {
    return {
      ...
      flowLabel: "New Request → Scheduled → En Route → On Site → Delivered",
      stages: ["New Request", "Scheduled", "En Route", "On Site", "Delivered"],
      labels: {
        item: "Property / Job",
        concern: "Requested Service",
        advisor: "Crew / Coordinator",
        eta: "Arrival / Service Delivery Time",
        phone: "Customer Phone",
        appointmentDate: "Service Date",
        appointmentTime: "Arrival Window",
        notes: "Service Special Instructions",
        stage: "Stage",
      },
      actions: {
        updateTitle: "Service update",
        updateSubtitle: "Your weekly meal plan is ready to review.",
  ...
  if (bt.includes("auto detailing")) {
    return {
      ...
      labels: {
        item: "Meal Plan / Service",
        concern: "Detail Request",
        advisor: "Detail Lead",
        eta: "Service Delivery Time",
        phone: "Customer Phone",
        appointmentDate: "Service Date",
        appointmentTime: "Arrival Window",
        notes: "Service Special Instructions",
...
function buildReadyMessage(job: RepairJob, businessName: string) {
  return `Hi ${job.customer || "customer"}, your ${job.vehicle || "job"} at ${businessName} is ready.${job.notes ? ` Special Instructions: ${job.notes}` : ""}`;
}
...
function buildStatusMessage(job: RepairJob, businessName: string) {
  return `Hi ${job.customer || "customer"}, here is your update from ${businessName}: ${job.vehicle || "Your job"} is currently in "${job.stage}".${job.concern ? ` Need: ${job.concern}.` : ""}${job.eta ? ` Delivery Time: ${job.eta}.` : ""}`;
}
...
function buildCampGuardianAlertMessage(job: RepairJob, businessName: string) {
  ...
  const note = job.notes ? ` Prep note: ${job.notes}.` : "";
...
function buildCampGuardianCheckoutMessage(job: RepairJob, businessName: string) {
  const childName = job.vehicle || "your child";
  return `Hi ${job.customer || "guardian"}, ${childName} has been checked out from ${businessName}.${job.notes ? ` Special Instructions: ${job.notes}` : ""}`;
}
...
export default function MealBusinessLiveDemo() {
...
const isActiveBoard =
  ...
  const actionConfig = config.actions || {
    updateTitle: "Weekly plan ready",
    updateSubtitle: "Your weekly meal plan is ready to review.",
    approvalTitle: "Prep update",
    approvalSubtitle: "Ask customer to approve the next step",
    appointmentTitle: "Delivery confirmation",
    appointmentSubtitle: "Confirm delivery day and drop-off window.",
  ...
  const restaurantSnapshot = useMemo(() => {
    ...
    const completedCount = jobs.filter(
      (job) => normalizeRestaurantStage(job.stage) === "Delivered",
    ).length;
      ...
      <div className="text-right text-[11px] uppercase tracking-[0.18em] text-white/40">
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-4">
          <StatCard
            ...
            value={totals.total}
          />
          <StatCard label="Packed" value={totals.inProgress} />
          <StatCard
            label={
              isRestaurant
                ? "Ready / Delivered"
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {isRestaurant
                    ...
                    : isCamp
                      ...
                      : "Click any card to open the client plan panel."}
                </p>
              </div>
              ...
              {selectedJob ? (
                <>
                  <div className="mb-5 flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="mt-1 text-sm text-slate-400">
                        {isCamp ? "Controlled guardian drawer" : "Working panel"}
                      </div>
                      {!isCamp ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <Field label={isCamp ? "Guardian Name" : "Client Name"}>
                      <input
                        value={
                          isCamp
                        ...
                        placeholder={isCamp ? "Guardian name" : "Client name"}
                      />
                    </Field>
                      ...
                      <div className="rounded-[24px] border border-violet-400/20 bg-violet-400/10 p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-2 text-sm leading-6 text-violet-50">
                              Turn this meal plan into visible proof. Capture planning, prep, packaging, and final delivery so the customer sees the week come together without confusion.
                            </div>
                        ...
                        <div className="mt-4">
                          ...
                          <textarea
                            ...
                            placeholder="What was planned, prepped, packed, or delivered?"
                          />
                        </div>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              addProofClip("Prep concern", "Immediate attention recommended before continuing.")
                            }
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              addProofClip("Delivered work", "Finished result verified and ready to show.")
                            }
                        ...
                        <div className="mt-4 rounded-[22px] border border-white/10 bg-[#070d1a] p-3">
                          ...
                          <div className="mt-3 space-y-2">
                            {selectedJob.proof?.length ? (
                              ...
                              <div className="rounded-2xl border border-dashed border-white/10 px-4 py-5 text-sm text-slate-500">
                                No meal moments captured yet. Start with planning, then log prep, packaging, and delivery.
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="mt-2 text-sm leading-6 text-cyan-50">
                            {isCamp
                              ...
                              : "Text the client directly or copy the message first."}
                          </div>
                        </div>
                      ...
                      {!selectedJob.phone ? (
                        <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-sm text-amber-100">
                          {isCamp
                            ...
                            : "Add a client phone number to enable text buttons."}
                        </div>
                      ) : null}
                    </div>
