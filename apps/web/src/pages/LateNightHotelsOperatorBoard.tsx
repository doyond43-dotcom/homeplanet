...
function toLateNightRequest(row: LateNightRequestRow): LateNightRequest {
  return {
    ...
    timing: row.timing,
    intent: row.intent,
    originalPropertyName: row.requested_property,
...
function LateNightHotelsOperatorBoard() {
  ...
  const [propertyAttempts, setPropertyAttempts] =
    useState<PropertyAttempt[]>([]);
  ...
  const loadPropertyAttempts = useCallback(
    async (requestId: string) => {
      const { data, error } = await supabase
        .from("late_night_hotel_property_attempts")
        .select(
          "id, request_id, property_name, quoted_price, status, created_at"
        )
      ...
      if (error) {
        console.error(
          "Property attempt history load failed:",
          error
        );
        setBoardError(
          "Unable to load property attempt history."
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    void loadPropertyAttempts(activeRequestId);
  }, [activeRequestId, loadPropertyAttempts]);
  ...
  async function updateStatus(id: string, status: RequestStatus) {
    ...
    if (
      currentRequest &&
      status !== "Matching Another Property"
    ) {
      ...
      if (status === "Contacting Property") {
        attemptPatch.contacted_at = now;
      }
      ...
      const { error: attemptError } = await supabase
        ...
        .update(attemptPatch)
        .eq("request_id", id)
        .eq("property_name", currentRequest.propertyName)
        .is("resolved_at", null);

      if (attemptError) {
        console.error(
          "Property attempt update failed:",
          attemptError
        );
      }

      void loadPropertyAttempts(id);
    }
  }
  ...
  async function assignNextProperty(id: string) {
    ...
    if (attemptError) {
      console.error(
        "Property attempt insert failed:",
        attemptError
      );
      setBoardError(
        "Property was assigned, but its attempt history could not be saved."
      );
    } else {
      void loadPropertyAttempts(id);
    }
            ...
            <div className="p-4 pb-8 sm:p-6">
              ...
              <section className="mt-4 rounded-3xl border border-white/10 bg-white/[0.04] p-5">
                ...
                <div className="mt-4 grid gap-3 text-sm">
                  <div className="flex items-start justify-between gap-4 border-b border-white/10 pb-3">
                    ...
                    <span className="max-w-[60%] text-right font-black">
                      {activeRequest.originalPropertyName}
                    </span>
                  </div>
                ...
                <div className="mt-3 flex items-end justify-end gap-4">
                  <div className="text-right">
                    ...
                    <div className="text-2xl font-black">
                      {formatRate(activeRequest.quotedPrice)}
                    </div>
                  </div>
                </div>
              ...
              <section className="mt-5 rounded-3xl border border-white/10 bg-white/[0.04] p-5">
                ...
                <div className="mt-4 grid gap-3">
                  {propertyAttempts.length === 0 ? (
                    <div className="text-sm text-white/45">
                      No property attempts recorded yet.
                    </div>
                  ) : (
                    propertyAttempts.map((attempt, index) => (
                      <div
                        key={attempt.id}
                      ...
                      >
                        ...
                        <span
                          className={`shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-black ${statusClasses(
                            attempt.status
                          )}`}
                        >
              ...
              <div className="mt-5 rounded-2xl border border-white/10 bg-black/10 p-4 text-sm leading-6 text-white/50">
                Operator actions are saved automatically and remain after
                refresh.
              </div>
            </div>
          </section>
