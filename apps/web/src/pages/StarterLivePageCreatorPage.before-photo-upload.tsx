...
type BlockType = "whoWeAre" | "pricingNote" | "servicePills" | "livePhotoWall" | "bookNow";
  ...
  function previewWrap(block: LiveBlock, children: React.ReactNode) {
    ...
    return (
      <div
        ...
        className={`cursor-pointer rounded-[1.75rem] transition ${
          isActive
            ? `ring-2 ${active.ring} ring-offset-2 ring-offset-[#07101d]`
  ...
  function renderSettings() {
      ...
      return (
        <div className="space-y-3">
          <p className="rounded-2xl border border-white/10 bg-black/30 p-4 text-sm leading-relaxed text-slate-400">
            Photo upload wiring comes next. For now, this block previews the Sebastian photo set.
          </p>
          <button className="flex h-12 w-full items-center justify-center rounded-2xl border border-dashed border-white/20 bg-black/30 text-sm font-semibold text-slate-300">
            Upload Photos
          </button>
        </div>
      );
    ...
    return (
      <div className="space-y-3">
        ...
        <p className="rounded-2xl border border-white/10 bg-black/30 p-4 text-sm leading-relaxed text-slate-400">
          Book Now will use this number for call/text actions when launch wiring is added.
        </p>
      </div>
    );
  }
