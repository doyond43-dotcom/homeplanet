...
export default function LivePagesHub() {
  ...
  const miniBoard: CSSProperties = {
    ...
    background:
      "linear-gradient(180deg, rgba(15,23,42,0.72), rgba(5,5,5,0.72))",
    padding: 16,
  };
      ...
      <div style={wrap}>
        ...
        <section style={hero} className="hp-live-offer-hero">
          <div style={heroCard}>
            ...
            <div style={actions}>
              ...
              <a href="/planet/live-pages/included" style={secondary}>
          ...
          <aside style={pricePanel}>
            <div>
              <div style={tinyLabel}>Starter Live Page System</div>
            ...
            <div>
              <div style={miniBoard}>
                <div style={tinyLabel}>LIVE WORK BOARD PREVIEW</div>

                {[
                  ["New request", "Sarah M."],
                  ["Photos uploaded", "3 files"],
                  ["Estimate needed", "Ready"],
                  ["Review status", "Building"],
                ].map(([left, right]) => (
                  <div key={left} style={miniRow}>
                    <span style={{ color: "#e5e7eb", fontWeight: 900 }}>{left}</span>
                    <span style={{ color: "#34d399", fontWeight: 950 }}>{right}</span>
                  </div>
                ))}
              </div>

              <p style={{ color: "#cbd5e1", lineHeight: 1.5, fontSize: 16, marginTop: 18 }}>
        ...
        <section style={strip}>
          ...
          <h2 style={{ ...h2, marginTop: 10 }}>
            Websites get looked at. Live Pages get used.
          </h2>
        ...
        <section id="included" style={strip}>
          ...
          <h2 style={{ ...h2, marginTop: 10 }}>One focused Live Page System for your main workflow.</h2>
          <p style={{ color: "#94a3b8", lineHeight: 1.55, maxWidth: 820, fontSize: 17 }}>
            This is not a dead website and it is not a random app. It is a focused Live Page System built around
        ...
        <section style={strip}>
          <div style={tinyLabel}>LIVE BUILD QUEUE</div>
          <h2 style={{ ...h2, marginTop: 10 }}>Built in a real queue, not a black hole.</h2>
          <p style={{ color: "#94a3b8", lineHeight: 1.55, maxWidth: 820, fontSize: 17 }}>
            When you message us, your request moves through a simple build flow. You are not guessing
            where things stand. We build in order, keep the workflow clear, and move each Live Page System
            toward review.
          </p>

          <div style={queueGrid}>
            {[
              ["01", "Message received", "Your spot is held in the build queue."],
              ["02", "Info confirmed", "We confirm what you do and what customers need."],
              ["03", "First version building", "Your Live Page and work board are shaped around your workflow."],
              ["04", "Ready for review", "You see the first version before paying."],
              ["05", "Approved and live", "$47/month keeps your Live Page System live and maintained."],
            ].map(([num, title, text]) => (
              <article key={num} style={queueStep}>
                <div style={tinyLabel}>{num}</div>
                <h3 style={{ margin: "10px 0 0", fontSize: 22, letterSpacing: "-0.04em" }}>{title}</h3>
                <p style={{ color: "#94a3b8", lineHeight: 1.42, marginBottom: 0 }}>{text}</p>
              </article>
            ))}
          </div>
        </section>
