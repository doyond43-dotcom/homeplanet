...
function LateNightHotelsLandingPage() {
  ...
  const [travelerRequestStatus, setTravelerRequestStatus] = useState("");
  const [travelerCurrentProperty, setTravelerCurrentProperty] = useState("");
  const [travelerConfirmedProperty, setTravelerConfirmedProperty] = useState("");
  const [travelerConfirmedRate, setTravelerConfirmedRate] = useState("");
  ...
  useEffect(() => {
    if (!activeRequestId || step !== "requested") return;

    const applyTravelerUpdate = (row: Record<string, unknown>) => {
      const status = String(row.status ?? "");
      const currentProperty = String(row.current_property ?? "");
      const confirmedProperty = String(row.confirmed_property ?? "");
      const confirmedRate = String(row.confirmed_rate ?? "");

      setTravelerRequestStatus(status);
      setTravelerCurrentProperty(currentProperty);
      setTravelerConfirmedProperty(confirmedProperty);
      setTravelerConfirmedRate(confirmedRate);
    };

    const channel = supabase
      .channel(`late-night-traveler-${activeRequestId}`)
      .on(
        "postgres_changes",
        {
          event: "UPDATE",
          schema: "public",
          table: "late_night_hotel_requests",
    ...
    const refreshTravelerRequest = async () => {
      const { data, error } = await supabase
        .from("late_night_hotel_requests")
        .select(
          "status, current_property, confirmed_property, confirmed_rate"
        )
      ...
      if (error) {
        console.error(
          "Traveler request status refresh failed:",
          error
        );
        return;
      }
  ...
  async function submitRoomRequest() {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, status, current_property, confirmed_property, confirmed_rate"
      )
      .single();
    ...
    const { error: initialAttemptError } = await supabase
      .from("late_night_hotel_property_attempts")
      .insert({
        request_id: data.id,
        property_name: selectedHotel.name,
        quoted_price: selectedHotel.price,
        status: "Needs Confirmation",
      });

    if (initialAttemptError) {
      console.error(
        "Initial property attempt insert failed:",
        initialAttemptError
      );
    }

    setActiveRequestId(data.id);
    setTravelerRequestStatus(data.status ?? "Needs Confirmation");
    setTravelerCurrentProperty(
      data.current_property ?? selectedHotel.name
    );
    setTravelerConfirmedProperty(data.confirmed_property ?? "");
    setTravelerConfirmedRate(data.confirmed_rate ?? "");
  ...
  const startNewSearch = () => {
    ...
    setTravelerRequestStatus("");
    setTravelerCurrentProperty("");
    setTravelerConfirmedProperty("");
    setTravelerConfirmedRate("");
          ...
          >
            ...
            <div className="p-4 pb-8 sm:p-6">
              ...
              {step === "requested" && selectedHotel && (
                <div>
                  {travelerRequestStatus === "Confirmed" ? (
                    <>
                      ...
                      <p className="mt-2 leading-6 text-white/60">
                        Your late-night room has been confirmed.
                      </p>

                      <div className="mt-6 rounded-3xl border border-emerald-400/20 bg-emerald-400/[0.06] p-5">
                        <div className="text-xs font-bold uppercase tracking-[0.17em] text-emerald-200/60">
                          Confirmed
                        </div>

                        <div className="mt-2 text-2xl font-black">
                          {travelerConfirmedProperty ||
                            travelerCurrentProperty ||
                            selectedHotel.name}
                        </div>

                        {travelerConfirmedRate && (
                          <div className="mt-2 text-lg font-black text-emerald-200">
                            {travelerConfirmedRate.startsWith("$")
                              ? travelerConfirmedRate
                              : `$${travelerConfirmedRate}`}
                          </div>
                        )}
                      ...
                      <div className="mt-5 rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.05] p-4">
                        ...
                        <p className="mt-2 text-sm leading-6 text-white/60">
                          Your confirmed room information is shown above.
                        </p>
                      </div>
                    </>
                  ) : travelerRequestStatus === "Declined" ? (
                    <>
                      <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/[0.07] text-[#f3c57d]">
                        <Clock3 className="h-7 w-7" />
                      </div>

                      <h2 className="mt-5 text-3xl font-black tracking-tight">
                        We couldn’t confirm a room yet.
                      </h2>

                      <p className="mt-2 leading-6 text-white/60">
                        We weren’t able to confirm a match for this request
                        right now.
                      </p>
                    ...
                    <>
                      ...
                      <p className="mt-2 leading-6 text-white/60">
                        We’re working on finding the best available room for
                        you.
                      </p>
                      ...
                      <div className="mt-5 rounded-2xl border border-white/10 bg-black/10 p-4">
                        <div className="font-black">You can relax.</div>

                        <p className="mt-2 text-sm leading-6 text-white/55">
                          We’ll update this screen when there’s something you
                          need to know.
                        </p>
                      </div>
                    </>
