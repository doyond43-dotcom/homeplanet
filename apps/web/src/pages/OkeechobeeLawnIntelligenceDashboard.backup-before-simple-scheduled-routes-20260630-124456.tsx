        ...
        <section className="mt-8 rounded-3xl border border-green-400/20 bg-green-400/[0.06] p-6 md:p-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm font-semibold leading-relaxed text-white/60">
                Plan yards by zone, day, or route note so Daniel, Brock, or anyone helping can see what is ready, scheduled, or completed.
              </p>
            </div>
          ...
          <div className="mt-6 grid gap-3">
            ...
            {!loading && routePlanRequests.length === 0 && (
              <div className="rounded-2xl border border-white/10 bg-black/30 p-5 text-sm font-semibold text-white/60">
                No route items yet. Mark a request Ready or Scheduled to move it into the route planner.
              </div>
            )}
          </div>
        </section>
...
What days or times usually work best, and is there anything else we should know before we come by?`;
  ...
  const routeStatuses = ["New", "Contacted", "Ready", "Scheduled", "Done"];
      ...
      <div className="mt-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <div className="mt-1 grid gap-2 md:grid-cols-2">
              <input
                ...
                placeholder="Zone: Oak Park, In Town, Treasure Island"
                className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-xs font-bold text-white outline-none placeholder:text-white/30 focus:border-green-400/50 md:min-w-[240px]"
              />

              <input
                value={routeDayDraft}
                onChange={(event) => setRouteDayDraft(event.target.value)}
                placeholder="Day / note: Wednesday, Thursday, Next Oak Park route"
                className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-xs font-bold text-white outline-none placeholder:text-white/30 focus:border-green-400/50 md:min-w-[240px]"
              />
            </div>
          </div>
          ...
          <button
            type="button"
            onClick={() =>
              onRouteUpdate(request, "Scheduled", routeZoneDraft, routeDayDraft)
            }
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-5">
          {routeStatuses.map((status) => (
            <button
              key={status}
              type="button"
              onClick={() => onRouteUpdate(request, status, routeZoneDraft, routeDayDraft)}
              className={[
