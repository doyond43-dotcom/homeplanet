...
export default function OkeechobeeLawnIntelligenceDashboard() {
  ...
  const todayRouteRequests = useMemo(
    () =>
      requestSummaries.filter((request) =>
        ["Contacted", "Scheduled", "On Route", "Done"].includes(
        ...
        <section className="mt-8 rounded-3xl border border-green-400/20 bg-green-400/[0.06] p-6 md:p-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm font-semibold leading-relaxed text-white/60">
                Requests moved past New show here so Daniel, Brock, or anyone helping can see what is being contacted, scheduled, routed, or completed.
              </p>
            </div>

            <div className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm font-black text-green-300">
              {todayRouteRequests.length} active
            </div>
          </div>

          <div className="mt-6 grid gap-3">
            {todayRouteRequests.map((request) => (
              <div
                key={request.id}
                className="rounded-3xl border border-white/10 bg-black/30 p-5"
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    <div className="text-xl font-black">{request.name}</div>
            ...
            {!loading && todayRouteRequests.length === 0 && (
              <div className="rounded-2xl border border-white/10 bg-black/30 p-5 text-sm font-semibold text-white/60">
                No route items yet. Use the status buttons on a request card to move someone into the route.
              </div>
            )}
          </div>
        </section>
        <section className="mt-8 rounded-3xl border border-green-400/20 bg-green-400/[0.05] p-6 md:p-8">
          ...
          <div className="mt-6 grid gap-4">
            {requestSummaries.map((request) => (
              <RequestSummaryCard key={request.id} request={request} onRouteUpdate={saveRouteUpdate} />
            ))}
...
What days or times usually work best, and is there anything else we should know before we come by?`;
  ...
  const routeStatuses = ["New", "Contacted", "Scheduled", "On Route", "Done"];
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <input
              ...
              placeholder="Oak Park, In Town, Treasure Island, etc."
              className="mt-2 w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-bold text-white outline-none placeholder:text-white/30 focus:border-green-400/50 md:min-w-[320px]"
            />
          </div>
          ...
          <button
            type="button"
            onClick={() =>
              onRouteUpdate(request, "Scheduled", routeZoneDraft)
            }
            className="rounded-2xl bg-green-400 px-5 py-3 text-sm font-black text-black"
          >
            Add To Route
          </button>
        </div>
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-5">
          {routeStatuses.map((status) => (
            <button
              key={status}
              type="button"
              onClick={() => onRouteUpdate(request, status, routeZoneDraft)}
              className={[
