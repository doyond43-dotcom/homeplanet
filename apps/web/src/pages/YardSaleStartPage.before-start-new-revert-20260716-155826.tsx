  ...
  const publishYardSale = async () => {
    ...
    try {
      ...
      for (const item of cleanItems) {
        ...
        if (item.imageData) {
          const itemResponse = await fetch(item.imageData);
          ...
          const allowedExtension = [
            ...
            ? originalExtension
            : itemBlob.type === "image/png"
              ? "png"
              : itemBlob.type === "image/webp"
                ? "webp"
                : "jpg";
          ...
          const itemUploadResult = await supabase.storage
            .from("yard-sale-images")
            .upload(itemPhotoPath, itemBlob, {
              cacheControl: "3600",
              contentType:
                itemBlob.type || "image/jpeg",
              upsert: false,
            });

          if (itemUploadResult.error) {
            throw new Error(
              `Item photo upload failed for ${
                item.name
              }: ${itemUploadResult.error.message}`,
            );
          }

          itemImageUrl = supabase.storage
            .from("yard-sale-images")
            .getPublicUrl(itemPhotoPath).data.publicUrl;
        }
          ...
          <button
            ...
            onClick={() => {
              const confirmed = window.confirm(
                "Start a new yard sale? This will clear everything currently saved in this builder. Your published yard sale pages will not be deleted.",
              );
                ...
                <div className="ys-item-row" key={index}>
                  ...
                  <label className="ys-item-photo-field">
                    ...
                    <div className="ys-item-photo-upload">
                      ...
                      {item.imageData ? (
                        <div
                          className="ys-item-photo-preview"
                          style={{
                            backgroundImage: `
                              linear-gradient(
                                180deg,
                        ...
                        >
                          <strong>
                            {item.imageName || "Item photo selected"}
                          </strong>
                        </div>
                      ) : (
                        <div className="ys-item-photo-empty">
                          <strong>Add a photo of this item</strong>
                          <small>
                            A clear item photo is what makes
                            shoppers stop and click.
                          </small>
                        </div>
                      )}
                    </div>
      ...
      <style>{`
        ...
        * {
        ...
        .ys-item-photo-upload {
          ...
          min-height: 132px;
          overflow: hidden;
          border: 1px dashed rgba(89, 255, 145, 0.24);
          border-radius: 15px;
          background: rgba(89, 255, 145, 0.025);
          cursor: pointer;
        }

        .ys-item-photo-upload > input {
          position: absolute;
          z-index: 2;
          inset: 0;
          width: 100%;
