...
function groupTicketsByTable(tickets: Ticket[], stage: TicketStage): TableGroup[] {
  ...
  return Array.from(groups.entries()).map(([table, groupTickets]) => {
    ...
    return {
      ...
      stage,
      hasAllergy: groupTickets.some(
        (ticket) => ticket.flags.includes("ALLERGY") || ticket.urgency === "critical"
      ),
...
function TableGroupCard({
  ...
  return (
    <article className={`rounded-3xl border p-5 ${tableClass(group)}`}>
      <button onClick={() => setIsOpen(!isOpen)} className="w-full text-left">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h3 className="mt-2 text-4xl font-black">{tableTime}</h3>

            <p className="mt-2 text-xl font-black">
              {group.tickets.length} Items
            </p>

            <p className="mt-2 text-sm font-black text-neutral-300">
              {completedCount} Complete / {remainingCount} Remaining / {specialCount} Special
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2 text-right">
            ...
            {group.hasUpdate && (
              <span className="rounded-full bg-amber-300 px-3 py-2 text-xs font-black text-black">
                UPDATED
              </span>
            )}

            <span className="rounded-full bg-black/40 px-3 py-2 text-xs font-black text-white">
              {isOpen ? "CLOSE" : "OPEN"}
            </span>
          </div>
        </div>
                    ...
                    <div className="flex-1">
                      <p
                        className={`text-lg font-black ${
                          ticket.completed ? "text-neutral-500 line-through" : "text-white"
                        }`}
                      >
                        Seat {index + 1} {ticket.item.replace(/^Seat\s+\d+:\s*/i, "")}
                      </p>
                      ...
                      {ticket.flags.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-2">
                          {ticket.flags.map((flag) => (
                            <span
                              key={flag}
                              className={`rounded-full px-2 py-1 text-[10px] font-black ${flagClass(flag)}`}
                            >
                              {flag}
                            </span>
                          ))}
      ...
      {!isOpen && (
        <button
          onClick={() => onMoveTable(group.table, group.stage, group.stage === "New" ? "Cooking" : "Ready")}
...
export default function BrahmaBullKitchenBoard() {
  ...
  function moveTable(table: string, fromStage: TicketStage, toStage: TicketStage) {
    const finalStage = fromStage === "Ready" && toStage === "Ready" ? "Delivered" : toStage;

    saveTickets(
      tickets.map((ticket) =>
        ticket.table === table && ticket.stage === fromStage
          ? { ...ticket, stage: finalStage, completed: finalStage === "Delivered" ? true : ticket.completed }
          : ticket
      )
    );
  }
