...
export default function PlanetOverview() {
  ...
  if (!planet) {
    return (
      <div style={{ padding: 18 }}>
        ...
        <p style={{ marginTop: 10, color: "rgba(255,255,255,0.70)" }}>That planet id doesn’t exist yet.</p>
      </div>
    );
  }
  ...
  const smallCard: React.CSSProperties = { ...cardBase, boxShadow: "0 10px 34px rgba(0,0,0,0.30)" };
      ...
      <div style={shellCard}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ marginTop: 8, color: "rgba(255,255,255,0.65)" }}>{(planet as any)?.subtitle ?? ""}</div>
            <div style={pill}>
        ...
        <div style={{ marginTop: 14 }}>
          <div style={grid as any}>
            <TapLink to={featured.to} style={featuredCard}>
            ...
            <div style={stack}>
              ...
              {secondaryCities.length === 0 ? (
                ...
                secondaryCities.slice(0, 6).map((c) => (
                  <TapLink key={c.id} to={c.to} style={smallCard}>
