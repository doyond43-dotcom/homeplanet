...
{(() => {
  ...
  const stages = ["Waiting", "En Route", "Picked Up", "Arrived", "Paid"];
  const normalizedStatus = selectedRide.status === "Collect Payment" ? "Arrived" : selectedRide.status;
  const currentIndex = Math.max(0, stages.indexOf(normalizedStatus));
  const markerLeft = `${(currentIndex / (stages.length - 1)) * 100}%`;
