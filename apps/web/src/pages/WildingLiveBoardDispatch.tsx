...
const dispatchStatusOptions: Array<{ id: DispatchStatus; label: string }> = [
  { id: "not_scheduled", label: "Not Scheduled" },
  { id: "scheduled", label: "Scheduled" },
  { id: "dispatched", label: "Dispatched" },
  { id: "en_route", label: "En Route" },
  { id: "on_site", label: "On Site" },
  { id: "reschedule_needed", label: "Reschedule Needed" },
];
  ...
  return [
    {
      ...
      assignedCrew: "Crew 1",
      dispatchTime: "08:50 AM",
      dispatchStatus: "scheduled",
      customerConfirmed: true,
      materialsReady: true,
      officeNotes: "Call before arrival. Gate side access.",
      stage: "scheduled",
      scopeItems: [
        {
          id: "scope-501-a",
    ...
    },
    {
      ...
      assignedCrew: "Crew 2",
      dispatchTime: "12:15 PM",
      dispatchStatus: "scheduled",
      customerConfirmed: true,
      materialsReady: false,
      officeNotes: "Customer prefers darker screen mesh if available.",
      stage: "measured",
      scopeItems: [
        {
          id: "scope-502-a",
    ...
    },
    {
      ...
      assignedCrew: "Crew 1",
      dispatchTime: "",
      dispatchStatus: "not_scheduled",
      customerConfirmed: false,
      materialsReady: false,
      officeNotes: "Waiting on approval before dispatch.",
      stage: "estimate_sent",
      scopeItems: [
        {
          id: "scope-503-a",
    ...
    },
    {
      ...
      customerConfirmed: true,
      materialsReady: true,
      officeNotes: "Material order placed. Watch for delivery timing.",
      stage: "ordered",
      scopeItems: [
        {
          id: "scope-504-a",
    ...
    },
    {
      ...
      customerConfirmed: true,
      materialsReady: true,
      officeNotes: "Installed clean. Waiting on final photos.",
      stage: "installed",
      scopeItems: [
        {
          id: "scope-505-a",
...
function buildTechText(job: Job) {
  return [
    ...
    `Dispatch Time: ${job.dispatchTime || "-"}`,
    `Dispatch Status: ${
      dispatchStatusOptions.find((x) => x.id === (job.dispatchStatus || "not_scheduled"))?.label || "Not Scheduled"
    }`,
    `Customer Confirmed: ${job.customerConfirmed ? "Yes" : "No"}`,
    `Materials Ready: ${job.materialsReady ? "Yes" : "No"}`,
    `Office Notes: ${job.officeNotes || "-"}`,
...
export default function WildingLiveBoardDispatch() {
          ...
          <header className="mb-4">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  type="button"
                  className="rounded-2xl border border-emerald-400/30 bg-emerald-500/25 px-5 py-2 text-sm font-semibold text-emerald-100 hover:bg-emerald-500/35"
              ...
              {selectedJob ? (
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className="rounded-2xl border border-cyan-400/30 bg-cyan-500/20 px-4 py-2 text-sm font-semibold text-cyan-100 hover:bg-cyan-500/30"
                  >
                    Copy Tech Text
                  </button>
                  ...
                  {selectedJob.phone ? (
                    <a
                      ...
                      className="rounded-2xl border border-emerald-400/30 bg-emerald-500/20 px-4 py-2 text-sm font-semibold text-emerald-100 hover:bg-emerald-500/30"
                    >
                      Send SMS
                    </a>
                  ) : null}
                  ...
                  {selectedJob.email ? (
                    <a
                      ...
                      className="rounded-2xl border border-violet-400/30 bg-violet-500/20 px-4 py-2 text-sm font-semibold text-violet-100 hover:bg-violet-500/30"
                    >
                      Email
                    </a>
                  ) : null}
              ...
              <div className="grid gap-4 xl:grid-cols-[1fr_1fr_1.05fr]">
                <div className="rounded-[24px] border border-white/10 bg-[#0c1623] p-4">
                  ...
                  <div className="mt-4 rounded-2xl border border-emerald-400/20 bg-emerald-500/5 p-3">
                    <div className="mb-3 text-sm font-semibold uppercase tracking-[0.14em] text-emerald-200">
                      Dispatch + Scheduling
                    </div>

                    <div className="space-y-3">
                      <div>
                        <div className="mb-1 text-xs text-white/50">Assigned Crew</div>
                        <input
                          value={selectedJob.assignedCrew}
                          onChange={(e) => updateSelected({ assignedCrew: e.target.value })}
                          className="w-full rounded-xl border border-white/10 bg-[#111d2e] px-3 py-2 text-sm text-white outline-none"
                        />
                      </div>

                      <div>
                        <div className="mb-1 text-xs text-white/50">Dispatch Time</div>
                        <input
                          value={selectedJob.dispatchTime ?? ""}
                          onChange={(e) => updateSelected({ dispatchTime: e.target.value })}
                          placeholder="8:30 AM leave shop"
                          className="w-full rounded-xl border border-white/10 bg-[#111d2e] px-3 py-2 text-sm text-white outline-none placeholder:text-white/30"
                        />
                      </div>

                      <div>
                        <div className="mb-1 text-xs text-white/50">Dispatch Status</div>
                        <select
                          value={selectedJob.dispatchStatus ?? "not_scheduled"}
                          onChange={(e) =>
                            updateSelected({ dispatchStatus: e.target.value as DispatchStatus })
                          }
                          className="w-full rounded-xl border border-white/10 bg-[#111d2e] px-3 py-2 text-sm text-white outline-none"
                        >
                      ...
                      <label className="flex items-center gap-2 text-sm text-white/85">
                        <input
                          type="checkbox"
                          checked={Boolean(selectedJob.customerConfirmed)}
                          onChange={(e) => updateSelected({ customerConfirmed: e.target.checked })}
                      ...
                      <label className="flex items-center gap-2 text-sm text-white/85">
                        <input
                          type="checkbox"
                          checked={Boolean(selectedJob.materialsReady)}
                          onChange={(e) => updateSelected({ materialsReady: e.target.checked })}
                          className="h-4 w-4 rounded border-white/20 bg-transparent"
                        />
                        Materials Ready
                      </label>
                      ...
                      <div>
                        <div className="mb-1 text-xs text-white/50">Office / Route Notes</div>
                        <textarea
                          value={selectedJob.officeNotes ?? ""}
                          onChange={(e) => updateSelected({ officeNotes: e.target.value })}
                          rows={3}
                          className="w-full rounded-xl border border-white/10 bg-[#111d2e] px-3 py-2 text-sm text-white outline-none placeholder:text-white/30"
                          placeholder="Gate code, customer callback note, route detail, parking, etc."
                        />
                      </div>
                    </div>
                  </div>
