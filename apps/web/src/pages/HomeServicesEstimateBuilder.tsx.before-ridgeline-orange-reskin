...
export default function HomeServicesEstimateBuilder() {
  ...
  const [spiderWebDensity, setSpiderWebDensity] = useState<Level>(() =>
    levelFromUrl(params.get("spiderWebDensity"))
  );
  const [limitedAccess, setLimitedAccess] = useState<Level>(() =>
    levelFromUrl(params.get("limitedAccess"))
  );
