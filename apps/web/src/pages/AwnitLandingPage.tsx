 ...
 * - Demo-week Ladder Mode: ALSO saves request into Supabase (public.awnit_leads)
 */
...
function mapUiProjectToDb(pt: ProjectType): DbProjectType {
  switch (pt) {
    case "Impact Windows":
      return "Windows";
    case "Hurricane Shutters":
      return "Other";
    case "Door Installations":
      return "Doors";
    case "Screen Repair":
      return "Repair";
    case "Trim + Caulk Finish":
      return "Repair";
    case "Custom Storm Solutions":
    default:
      return "Other";
  }
}
...
export default function AwnitLandingPage() {
  ...
  async function saveLeadToDb() {
    ...
    try {
      const payload = {
        ...
        email: null,
        address: (quote.address || "").trim() || null,
        project_type: mapUiProjectToDb(quote.projectType),
        best_time: null,
        notes: [
          `UI Project Type: ${quote.projectType}`,
          ``,
          (quote.notes || "").trim(),
        ]
          .join("\n")
      ...
      setSaveOk("Saved ✅ (Ladder Mode captured)");
  ...
  async function handleSaveThenOpenDraft(kind: "email" | "sms") {
    const res = await saveLeadToDb();
    if (!res.ok) return;

    // Demo-week behavior: after saving, open the draft for the customer to send.
    if (kind === "email") {
      if (mailto) window.location.href = mailto;
      else if (textHref) window.location.href = textHref;
    } else {
      if (textHref) window.location.href = textHref;
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-white/65">
                Demo-week Ladder Mode: this saves your request instantly (no voicemail), and you can still send it by email/text if you want.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={() => handleSaveThenOpenDraft("email")}
                disabled={saving}
                className={cn("rounded-xl bg-emerald-500/90 px-4 py-3 text-sm font-extrabold text-black hover:bg-emerald-400", saving && "opacity-70 cursor-not-allowed")}
                title={mailto ? "Save + open email draft" : "Save + open text draft"}
              >
            ...
            <div className="rounded-3xl border border-white/10 bg-black/20 p-5 md:p-6">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="mt-2 text-[11px] text-white/45">
                    Saved as: <span className="font-bold">{mapUiProjectToDb(quote.projectType)}</span> (database category)
                  </div>
                </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button
                  ...
                  disabled={saving}
                  className={cn("rounded-xl bg-emerald-500/90 px-5 py-3 text-sm font-extrabold text-black hover:bg-emerald-400", saving && "opacity-70 cursor-not-allowed")}
                  title="Save to AWNIT board (no email/text needed)"
                >
                ...
                <button
                  type="button"
                  onClick={() => handleSaveThenOpenDraft("email")}
                  disabled={saving || (!mailto && !textHref)}
                  className={cn(
                    "rounded-xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-extrabold hover:bg-white/10",
                    (saving || (!mailto && !textHref)) && "opacity-60 pointer-events-none"
                  )}
                  title={!mailto && !textHref ? "Set BUSINESS.phoneDisplay / BUSINESS.email" : mailto ? "Save + open email draft" : "Save + open text draft"}
                >
                  Save + Email/Text
                </button>
                ...
                <button
                  type="button"
                  onClick={() => handleSaveThenOpenDraft("sms")}
                  disabled={saving || (!textHref && !mailto)}
                  className={cn(
                    "rounded-xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-extrabold hover:bg-white/10",
                    (saving || (!textHref && !mailto)) && "opacity-60 pointer-events-none"
                  )}
                  title={textHref ? "Save + open text draft" : "Save + open email draft"}
                >
                  Save + Text
                </button>
              </div>
              ...
              <div className="mt-3 text-xs text-white/45">
                Ladder Mode: this saves instantly to the AWNIT board so Austin can review safely after the job.
              </div>
            </div>
