                  ...
                  <div className="rounded-2xl border border-emerald-300/15 bg-emerald-400/10 p-3">
                    <div className="mb-3 flex items-center justify-between gap-3">
                      ...
                      <span className="text-xs font-semibold text-white/45">
                        {openCustomer.status === "paid"
                          ? "Done"
                          : !openCustomer.invoice
                            ? "Invoice needed"
                            : openCustomer.status === "sent"
                              ? "Waiting payment"
                              : "Ready to send"}
                      </span>
                    </div>
                    ...
                    {openCustomer.status === "paid" ? (
                      <button
                        disabled
                        className="w-full rounded-xl bg-white/15 px-3 py-3 text-sm font-black text-white/60"
                      >
                        Paid / Done
                      </button>
                    ) : !openCustomer.invoice ? (
                      <button
                        onClick={() => createInvoice(openCustomer)}
                        className="w-full rounded-xl bg-purple-400/20 px-3 py-3 text-sm font-black text-purple-100"
                      >
                        Create Invoice
                      </button>
                    ) : openCustomer.status === "sent" ? (
                      <button
                        onClick={() =>
                          updateCustomer(openCustomer.id, {
                            status: "paid",
                            invoice: openCustomer.invoice
                              ? { ...openCustomer.invoice, status: "paid", updatedAt: nowIso() }
                              : openCustomer.invoice,
                          })
                        }
                        className="w-full rounded-xl bg-white px-3 py-3 text-sm font-black text-black"
                      >
                        Mark Paid
                      </button>
                    ) : (
                      <button
                        onClick={() => sendInvoice(openCustomer)}
                        className="w-full rounded-xl bg-emerald-400 px-3 py-3 text-sm font-black text-black"
                      >
                        Send Invoice
                      </button>
                    )}
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-2">
                      ...
                      <button
                        ...
                        className="rounded-xl border border-emerald-300/15 bg-emerald-400/10 px-3 py-2.5 text-xs font-bold text-emerald-100"
                      >
                        Proof
                      </button>
                    </div>
