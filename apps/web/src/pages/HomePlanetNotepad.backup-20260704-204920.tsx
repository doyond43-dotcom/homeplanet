...
export default function HomePlanetNotepad() {
  ...
  const [openSection, setOpenSection] = useState<DrawerSection>("invoice");
  ...
  function openCustomerDrawer(customer: Customer, section: DrawerSection = "invoice") {
      ...
      <main className="relative mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <section className="rounded-[2rem] border border-white/10 bg-black/45 p-5 shadow-2xl shadow-emerald-950/30 backdrop-blur md:p-8">
          ...
          <div className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr] lg:items-end">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              <a
                href="#add-customer"
                className="rounded-2xl bg-emerald-400 px-5 py-4 text-center text-base font-black text-black shadow-lg shadow-emerald-400/20"
              >
                Add Customer
              </a>
              <a
                href="#open-charges"
                className="rounded-2xl border border-white/15 bg-white/10 px-5 py-4 text-center text-base font-black text-white"
              >
                View Open Charges ({openCharges})
              </a>
            </div>
          </div>
        </section>
        ...
        <section id="add-customer" className="rounded-[1.6rem] border border-white/10 bg-white/[0.035] p-4 shadow-xl shadow-black/20">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-white/45 hover:border-emerald-300/30 hover:text-emerald-100"
                type="button"
              >
                Reset Demo
              </button>

              <div className="hidden rounded-full border border-emerald-300/15 bg-emerald-400/10 px-3 py-1 text-xs font-black uppercase tracking-[0.16em] text-emerald-100 sm:block">
                Notepad Mode
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-3 lg:grid-cols-[1fr_0.75fr_0.55fr]">
            ...
            <input
              className="rounded-2xl border border-white/10 bg-black/40 px-4 py-3.5 text-white outline-none placeholder:text-white/35 focus:border-emerald-300/50 lg:col-span-2"
            ...
            <input
              ...
              onChange={(event) => setForm((prev) => ({ ...prev, paymentLink: event.target.value }))}
            />
          </div>

          <button
            onClick={saveCustomer}
            className="mt-3 w-full rounded-2xl bg-emerald-400 px-5 py-3.5 text-base font-black text-black shadow-lg shadow-emerald-400/20"
          >
            Save Customer
          </button>
        </section>
        ...
        <section id="open-charges" className="grid gap-4 lg:grid-cols-[1fr_420px]">
          <div className="flex flex-col gap-4 self-start">
            {customers.map((customer) => (
              <article
                key={customer.id}
                className="rounded-[1.75rem] border border-white/10 bg-black/45 p-4 shadow-xl shadow-black/20 transition hover:border-emerald-300/25 hover:bg-emerald-400/[0.035]"
              >
                ...
                {customer.invoice && (
                  <div className="mt-3 rounded-xl border border-purple-300/15 bg-purple-400/10 px-3 py-2 text-xs font-bold text-purple-100">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
                  <button onClick={() => textCustomer(customer)} className="rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-sm font-bold">
                    Text
                  </button>
                  <button onClick={() => createInvoice(customer)} className="rounded-xl border border-purple-300/20 bg-purple-400/15 px-3 py-3 text-sm font-bold text-purple-100">
                    Invoice
                  </button>
                  <button onClick={() => sendInvoice(customer)} className="rounded-xl border border-emerald-300/20 bg-emerald-400/15 px-3 py-3 text-sm font-bold text-emerald-100">
                    Send
                  </button>
                  <button onClick={() => updateCustomer(customer.id, { status: "paid", invoice: customer.invoice ? { ...customer.invoice, status: "paid", updatedAt: nowIso() } : customer.invoice })} className="rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-sm font-bold">
                    Paid
                  </button>
                  <button onClick={() => openCustomerDrawer(customer)} className="rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-sm font-bold">
                    Open
                  </button>
                </div>
              </article>
          ...
          <aside className="rounded-[1.75rem] border border-white/10 bg-white/[0.045] p-4 lg:sticky lg:top-4 lg:h-fit">
            {openCustomer ? (
              <div>
                <div className="mb-4 flex items-start justify-between gap-3">
                  ...
                  <span className={`rounded-full border px-3 py-1 text-xs font-black ${statusClass(openCustomer.status)}`}>
                    {statusLabel(openCustomer.status)}
                  </span>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <label className="grid gap-2">
                      ...
                      <input
                        ...
                        placeholder="Cash App, Square..."
                      />
                    </label>
                  </div>

                  <div className="rounded-2xl border border-emerald-300/15 bg-emerald-400/10 p-3">
                    <p className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-emerald-100/70">Next Action</p>
                    <div className="grid grid-cols-2 gap-2">
                      <button onClick={() => createInvoice(openCustomer)} className="rounded-xl bg-purple-400/15 px-3 py-3 text-sm font-bold text-purple-100">Create Invoice</button>
                      <button onClick={() => sendInvoice(openCustomer)} className="rounded-xl bg-emerald-400 px-3 py-3 text-sm font-black text-black">Send Invoice</button>
                      <button onClick={() => {
                        updateCustomer(openCustomer.id, { status: "followup" });
                        setOpenSection("timeline");
                      }} className="rounded-xl bg-amber-400/15 px-3 py-3 text-sm font-bold text-amber-100">Follow Up</button>
                      <button onClick={() => updateCustomer(openCustomer.id, { status: "paid", invoice: openCustomer.invoice ? { ...openCustomer.invoice, status: "paid", updatedAt: nowIso() } : openCustomer.invoice })} className="rounded-xl bg-white px-3 py-3 text-sm font-black text-black">Mark Paid</button>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-2">
                    <button
                      ...
                      className={`flex items-center justify-between rounded-2xl border px-4 py-3 text-left ${
                        openSection === "invoice"
                          ? "border-purple-300/25 bg-purple-400/15"
                      ...
                      <div className="rounded-2xl border border-purple-300/20 bg-purple-400/10 p-4">
                        {openCustomer.invoice ? (
                          <>
                            ...
                            <pre className="mt-3 max-h-32 overflow-auto whitespace-pre-wrap rounded-2xl border border-white/10 bg-black/35 p-3 text-xs leading-5 text-white/70">
                              {buildInvoiceText(openCustomer)}
                            </pre>

                            <button
                              onClick={() => copyInvoice(openCustomer)}
                              className="mt-3 w-full rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-sm font-bold"
                            >
                    ...
                    >
                      <span>
                        <span className="block text-sm font-black text-white">Proof Photos</span>
                        <span className="text-xs text-white/45">
