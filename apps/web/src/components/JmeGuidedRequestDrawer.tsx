...
const jmeSupabaseUrl =
  import.meta.env.VITE_SUPABASE_URL as string | undefined;

const jmeSupabaseAnonKey =
  import.meta.env.VITE_SUPABASE_ANON_KEY as string | undefined;

if (!jmeSupabaseUrl || !jmeSupabaseAnonKey) {
  throw new Error(
    "Supabase env missing for JME public request flow."
  );
}

const jmePublicSupabase = createClient(
  jmeSupabaseUrl,
  jmeSupabaseAnonKey,
  {
    auth: {
...
export default function JmeGuidedRequestDrawer({
  ...
  async function persistRentalRequest() {
    const { error } = await jmePublicSupabase.from("jme_requests").insert({
      request_type: "rental",
      status: "new",
      customer_name: rental.name.trim(),
      customer_phone: rental.phone.trim(),
      equipment: rental.equipmentNeed.trim(),
      brand_model: null,
      requested_start_date: rental.startDate,
      requested_return_date: rental.endDate,
      movement_preference: rental.movementPreference,
      project_details: rental.projectDetails.trim(),
      project_location: rental.location.trim(),
      problem_description: null,
      equipment_condition: null,
      service_preference: null,
      equipment_location: null,
      photo_names: [],
      source: "jme_live_page",
    });
  ...
  async function persistRepairRequest() {
    const { error } = await jmePublicSupabase.from("jme_requests").insert({
      request_type: "repair",
      status: "new",
      customer_name: repair.name.trim(),
      customer_phone: repair.phone.trim(),
      equipment: repair.equipmentType.trim(),
      brand_model: repair.brandModel.trim() || null,
      requested_start_date: null,
      requested_return_date: null,
      movement_preference: null,
      project_details: null,
      project_location: null,
      problem_description: repair.problem.trim(),
      equipment_condition: repair.currentCondition,
      service_preference: repair.servicePreference,
      equipment_location: repair.location.trim(),
      photo_names: repair.photoNames,
      source: "jme_live_page",
    });
  ...
  async function submitRequest() {
    ...
    try {
      if (isRental) {
        await persistRentalRequest();
        sendRentalRequest();
        return;
      }

      await persistRepairRequest();
      sendRepairRequest();
    } catch (requestError) {
      console.error("JME request save failed:", requestError);

      setError(
        "We could not save your request yet. Please try again or call or text JME directly."
      );
    }
  }
